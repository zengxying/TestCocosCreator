...
import { GlobalConst } from '../../../core_tgx/tool/GlobalConst';
...
@menu('game/game.MonsterBloodBar')
export class MonsterBloodBar extends Component {
    ...
    private _scriptParent: EntityComp = null!;//血条所在节点绑定的脚本
     ...
     */
    public show (scriptParent: EntityComp, totalBlood: number, offsetPos: Vec3, hpAddition: number, callback?: Function | null, isInit: boolean = true) {
    ...
    public refreshBlood (num: number) {
        ...
        if (num < 0) {//减血
            ...
            if (!this._isBloodEmpty) {
                ...
                if (this._isBloodEmpty) {
                    this._scriptParent.entity.isDie = true;
                }
            } else {
                PoolManager.instance.putNode(this.node);
                this._scriptParent.entity.isDie = true;
    ...
    update () {
        if (this.node.parent && this.node.active && this._ndTarget && this._ndTarget.parent) {
            ...
            GlobalConst.camera?.convertToUINode(worPos, this.node.parent, this._curPos);
