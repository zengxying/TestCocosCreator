...
export const RoundBoxAssemblerSuper: IAssembler = {

    // 根据圆角segments参数，构造网格的顶点索引列表
    GetIndexBuffer(sprite:RoundBox) {
        let indexBuffer = [
            0, 1, 2, 2, 3, 0,
            4, 5, 6, 6, 7, 4,
            8, 9, 10, 10, 11, 8
    ...
    createData (sprite: RoundBox) {
        ...
        let indexBuffer = this.GetIndexBuffer(sprite);
        renderData.chunk.setIndexBuffer(indexBuffer);
        return renderData;
    },

    // 照抄simple的
    updateRenderData (sprite: RoundBox) {
    ...
    // 局部坐标转世界坐标 照抄的，不用改
    updateWorldVerts (sprite: RoundBox, chunk: { vb: any; }) {
        const renderData = sprite.renderData!;
